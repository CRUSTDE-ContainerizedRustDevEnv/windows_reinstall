...
# ~/.ssh/sshadd.sh
# Inside this template, replace the words github_com_ssh_1 and bestia_dev_ssh_1 
...
cat ~/.ssh/config | grep -q "~/.ssh/localhost_2201_rustdevuser_ssh_1" || echo "The ~/.ssh/config does not contain the identity ~/.ssh/localhost_2201_rustdevuser_ssh_1."
cat ~/.ssh/config | grep -q "~/.ssh/github_com_ssh_1" || echo "The ~/.ssh/config does not contain the identity ~/.ssh/github_com_ssh_1."
cat ~/.ssh/config | grep -q "~/.ssh/bestia_dev_ssh_1" || echo "The ~/.ssh/config does not contain the identity ~/.ssh/bestia_dev_ssh_1."
